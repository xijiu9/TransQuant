...
from .image_classification.quantize import QLinear, QIdentity
...
class BertSelfAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None, builder=None):
        super().__init__()
...
class BertAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None, builder=None):
        super().__init__()
        self.self = BertSelfAttention(config, position_embedding_type=position_embedding_type, builder=builder)
        self.output = BertSelfOutput(config, builder)
...
class BertLayer(nn.Module):
    def __init__(self, config, builder=None):
        ...
        self.seq_len_dim = 1
        self.attention = BertAttention(config, builder=builder)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            if not self.is_decoder:
                raise ValueError(f"{self} should be used as a decoder model if cross attention is added")
            self.crossattention = BertAttention(config, position_embedding_type="absolute", builder=builder)
        self.intermediate = BertIntermediate(config, builder)
        self.output = BertOutput(config, builder)
...
class BertEncoder(nn.Module):
    def __init__(self, config, builder=None):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([BertLayer(config, builder) for _ in range(config.num_hidden_layers)])
...
class BertModel(BertPreTrainedModel):
    ...
    def __init__(self, config, add_pooling_layer=True, builder=None):
        super().__init__(config)
        self.config = config

        self.embeddings = BertEmbeddings(config, builder)
        self.encoder = BertEncoder(config, builder)

        self.pooler = BertPooler(config, builder) if add_pooling_layer else None
    ...
    def forward(
        ...
        past_key_values (`tuple(tuple(torch.FloatTensor))` of length `config.n_layers` with each tuple having 4 tensors of shape `(batch_size, num_heads, sequence_length - 1, embed_size_per_head)`):
            ...
            don't have their past key value states given to model) of shape `(batch_size, 1)` instead of all
...
bert_choices = {
    'classic': 'quantize nothing',
    'embedding': 'quantize BertEmbeddings',
    'attention': 'quantize BertSelfAttention',
...
# config是自行配置参数，bertConfig是bert自身的配置参数
def build_bert(version, config, bertConfig, model_state=None):
    version = bert_versions[version]
    config = bert_configs[config]

    builder = BertBuilder(version, config)
...
# Todo:根据bert源码解读暂定将量化层分为以下五类：
# 填充层：embedding
# selfAttention机制中涉及的三个变换矩阵：linearAttention
# selfoutput与output中与add & layernorm相关的线性层：linearAddNorm
# feed-forward相关的线性层：linearFeedForward
# 最终分类器相关的线性层：linearClassfier
